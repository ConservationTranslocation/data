...
/*=============================================================================================================*/	
		...
		$("#long_pop").on('focusout', function(evt) {
			coord_x = $("#long_pop").val();
		});
		
		$("#lat_pop").on('focusout', function(evt) {
			coord_y = $("#lat_pop").val();
		});
		...
		var addInteraction = function (geomtype) {
			...
			if (featureType === 'Point') {
				...
				map.on('click', function(evt) {
					...
					$("#long_pop").val(coord2[0].toFixed(6)); // arrondi à la 6e décimale
					$("#lat_pop").val(coord2[1].toFixed(6));
				});
			} 
			else {
				map.removeInteraction(draw);
				map.on('click', function(evt) {
					$("#long_pop").val(coord_x);
					$("#lat_pop").val(coord_y);
				});
			}
		};
		...
		/* Bouton qui permet de supprimer un point et les coordonnées associées */
		$('#delete_draw').on("click", function (e) {
			if(val!="empty"){
				lyr_pop.setVisible(false); // on s'assure de cacher le point qui viendrait de la BDD en faisant un test pour ne le faire que si besoin
			}removeLastFeature();
		...
		$('#valid_draw').on("click", function () {
			/* Si les champs ne sont pas vides */
			if($("#long_pop").val()!="" && $("#lat_pop").val()!=""){
			
				/* On récupère leur valeur que l'on parse en float*/
				var lon = parseFloat(document.getElementById('long_pop').value);
				var lat= parseFloat(document.getElementById('lat_pop').value);
